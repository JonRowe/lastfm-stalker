Given /^there is a last fm user "([^"]*)" currently listening to "([^"]*)" by "([^"]*)"$/ do |user, track, artist|
  ...
  stub_request(:get, uri).with( :query => { :method => method, :user => user, :format => 'json', :api_key => api_key } ).to_return( :body => json, :status => 200, :headers => { "Content-type" => 'application/json' } )
end
